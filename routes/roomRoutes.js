...
const {getAllRooms,createNewRoom,getRoomById,updateRoom, deleteRoom,getRoomsByBuildingId, deleteRoomById}= roomController

/**
 * This route is a red flag please modify it ones development is done.
 */
router.route('/deleteRoom').delete(authController.protect,authController.restrictTo('admin'), deleteRoom)
router.route('/').get(authController.protect, getAllRooms).post(authController.protect,authController.restrictTo('admin'),createNewRoom);
